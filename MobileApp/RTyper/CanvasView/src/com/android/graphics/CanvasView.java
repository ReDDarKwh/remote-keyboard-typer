...
public class CanvasView extends View {
    ...
    private INewPathCallback newPathCallback;
    private IMoveCallback moveCallback;
    ...
    public final int drawZoneWidth = 800;
    public final int drawZoneHeight = 300;



    public void onNewPath(INewPathCallback callback){

        this.newPathCallback = callback;
    }

    public void onMovePath(IMoveCallback callback){
    ...
    private float getPosX(MotionEvent event){

        return Math.max(0,Math.min(drawZoneWidth,(event.getX() - mPosX )/ mScaleFactor));
    }

    private float getPosY(MotionEvent event){

        return Math.max(0,Math.min(drawZoneHeight,(event.getY() - mPosY )/ mScaleFactor));
    }
    ...
    private Path createPath(MotionEvent event) {
        ...
        this.startX = getPosX(event);
        this.startY = getPosY(event);
        ...
        switch (this.mode) {
            ...
            case ERASER :
                if ((this.drawer != Drawer.QUADRATIC_BEZIER) && (this.drawer != Drawer.QUBIC_BEZIER)) {
                    ...
                    moveCallback.callback(this.startX, this.startY);

                    newPathCallback.callback(true);
                } else {
                    // Bezier
    ...
    private int cooldown = Integer.MAX_VALUE;
    /**
    ...
    private void onActionMove(MotionEvent event) {
            ...
            if(cooldown>1){
            float x = getPosX(event);
            float y = getPosY(event);
    ...
    private void onActionUp(MotionEvent event) {
        if (isDown) {
            ...
            newPathCallback.callback(false);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        paint.setColor(Color.rgb(47,167,212));
    ...
    public void setMoveMode(boolean moveMode) {
        ...
        Toast toast = Toast.makeText(getContext(), "Mode mode :" + (moveMode?" on ":" off "), Toast.LENGTH_SHORT);
        toast.show();
...
//            mPosX = mPosX - (detector.getFocusX() - focusX);
//            mPosY = mPosY - (detector.getFocusY() - focusY);
